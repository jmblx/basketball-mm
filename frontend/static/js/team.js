var teamSlug = new URL(window.location.href).pathname.split("/").pop();
       ...
       if (window.SharedWorker) {
        ...
        async function getTeamData() {
            const response = await fetch(`http://176.109.110.111/team/data/${teamSlug}`);
            if (response.ok) {
        ...
        async function getTeamImage() {
            fetch(`http://176.109.110.111/team/image/${teamId}`)
            .then(response => {
                if (!response.ok) {
        ...
        async function joinTeam() {
            const token = localStorage.getItem("auth_token");
            const response = await fetch(`http://176.109.110.111/team/join-team?team_id=${teamId}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
        ...
        async function saveChanges() {
            ...
            console.log(changes)
            const token = localStorage.getItem("auth_token");
            const response = await fetch(`http://176.109.110.111/team/update/${teamId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
        ...
        async function leaveTeam() {
            const token = localStorage.getItem("auth_token");
            const response = await fetch(`http://176.109.110.111/team/leave-team?team_id=${teamId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
