async function registerAndLogin(email, password, name) {
  try {
    // Попытка регистрации
    const registerResponse = await fetch('https://localhost:8082/auth/register', {
      method: 'POST',
      headers: {
    ...
    const loginResponse = await fetch('https://localhost:8082/auth/jwt/login', {
      method: 'POST',
      headers: {
...
async function fetchGoogleAuthUrl() {
  try {
    const response = await fetch('https://localhost:8082/auth/google/authorize');
    if (!response.ok) {
