...
async def find_opponent(
    user_id: UUID, user_rating: int, threshold: int, redis
):
...
async def start_search(
    ...
    background_tasks.add_task(
        search_for_match, user_id, user.solo_rating, redis
    )
        ...
        match = (
            (
                await session.execute(
                    select(SoloMatch)
                    .options(selectinload(SoloMatch.players))
...
async def confirm_ready(
    match_id: int, player_id: UUID, redis=Depends(get_redis)
):
    player_id_str = str(player_id)
    is_user1 = await redis.sismember(
        f"solomatch:{match_id}:user1", player_id_str
    )
    is_user2 = await redis.sismember(
        f"solomatch:{match_id}:user2", player_id_str
    )
    ...
    is_confirmed = await redis.sismember(
        f"solomatch:{match_id}:confirmed_players", player_id_str
    )
    if is_confirmed:
    ...
    confirmed_players = await redis.scard(
        f"solomatch:{match_id}:confirmed_players"
    )
...
async def not_ready(
    ...
    await notify_user(
        str(user_id), "matchCancelled", "Match cancelled by you."
    )
    await notify_user(
        str(opposing_user_id),
        "matchResearch",
        "Match cancelled by your opp. Searching for a new match.",
    )
    if opposing_user:
        await add_user_to_search(
            opposing_user_id, opposing_user.solo_rating, redis
        )
