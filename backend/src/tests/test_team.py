...
class TestTeam:
    ...
    async def test_create_team(
        ...
        response = await ac.post(
            ...
            headers={
                "Authorization": f"Bearer {authenticated_data.get('access_token')}"
            },
        )
    ...
    async def test_join_team(
        self, ac: AsyncClient, authenticated_data: dict, team_mock
    ):

        async with async_session_maker() as session:
            print(
                "Есть команда:",
                (await session.execute(select(Team))).scalar_one().id,
            )
        response = await ac.post(
            "/team/join-team",
            headers={
        ...
        async with async_session_maker() as session:
            stmt = (
                select(Team)
                .where(Team.id == team_mock)
                .options(selectinload(Team.players))
            )
            team = (await session.execute(stmt)).scalar()
            assert authenticated_data.get("user_data").get("id") in [
                str(p.id) for p in team.players
            ]
